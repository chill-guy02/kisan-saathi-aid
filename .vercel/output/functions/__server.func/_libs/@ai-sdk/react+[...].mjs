...
import { $ as normalizeHeaders, A as fetchWithValidatedRedirects, At as looseObject, B as isExecutableTool, Ct as _null, Dt as discriminatedUnion, Et as custom, F as getToolCaller, Ft as string, Gt as UnsupportedFunctionalityError, H as isNonNullable, It as union, J as lazySchema, Kt as getErrorMessage, L as isAbortError, Lt as unknown, M as generateId, Mt as number, Nt as object, O as detectMediaType, Ot as lazy, P as getRuntimeEnvironmentUserAgent, Pt as record, R as isBuffer, Rt as AISDKError, St as _instanceof, Tt as boolean, U as isProviderReference, V as isFullMediaType, Vt as InvalidPromptError, W as isProviderStreamError, Wt as TypeValidationError, at as resolve, b as createIdGenerator, c as DownloadError, ct as retryWithExponentialBackoff, d as cancelResponseBody, et as parseJsonEventStream, gt as withUserAgentSuffix, h as convertBase64ToUint8Array, it as readResponseWithSizeLimit, j as filterNullable, jt as never, k as executeTool, kt as literal, l as asArray, lt as safeParseJSON, mt as validateTypes, n as GatewayError, o as DEFAULT_MAX_DOWNLOAD_SIZE, q as isUrlSupported, r as gateway, s as DelayedPromise, t as GatewayAuthenticationError, u as asSchema, ut as safeValidateTypes, v as convertUint8ArrayToBase64, vt as zodSchema, wt as array, xt as _enum, zt as APICallError } from "./gateway+[...].mjs";
...
function getGlobalProvider() {
	var _a24;
	return asProviderV4((_a24 = globalThis.AI_SDK_DEFAULT_PROVIDER) != null ? _a24 : gateway);
}
...
function getMediaTypeFromUrl(url, fallbackMediaType = "application/octet-stream") {
	var _a24;
	try {
		const fileExtension = (_a24 = new URL(url).pathname.split(".").pop()) == null ? void 0 : _a24.toLowerCase();
...
function addLanguageModelUsage(usage1, usage2) {
	...
	return {
		...
		inputTokenDetails: {
			noCacheTokens: addTokenCounts((_a24 = usage1.inputTokenDetails) == null ? void 0 : _a24.noCacheTokens, (_b = usage2.inputTokenDetails) == null ? void 0 : _b.noCacheTokens),
...
function asAsyncIterableStream(stream) {
	stream[Symbol.asyncIterator] = function() {
		...
		async function cleanup(cancelStream) {
			...
			finished = true;
			try {
				if (cancelStream) await ((_a24 = reader.cancel) == null ? void 0 : _a24.call(reader));
			} finally {
				try {
					reader.releaseLock();
				} catch (e) {}
...
function createStitchableStream() {
	...
	const processPull = async () => {
		...
		try {
			...
			if (isCancelled) return;
			(_a24 = currentStream.onError) == null || _a24.call(currentStream, error);
			controller?.error(error);
			innerStreams.shift();
			terminate();
		}
	...
	return {
		...
		addStream: (innerStream, callbacks) => {
			var _a24;
			if (isCancelled) {
				(_a24 = callbacks == null ? void 0 : callbacks.onCancel) == null || _a24.call(callbacks);
	...
	*/
	...
	get output() {
		return this.finalStep.then((step) => {
			var _a24;
			return ((_a24 = this.outputSpecification) != null ? _a24 : text()).parseCompleteOutput({ text: step.text }, {
				response: step.response,
				usage: step.usage,
				finishReason: step.finishReason
			});
		...
		case "user":
			modelMessages.push({
				...
				content: message.parts.map((part) => {
					...
					if (isDataUIPart(part)) return (_a24 = options == null ? void 0 : options.convertDataPart) == null ? void 0 : _a24.call(options, part);
...
var AbstractChat = class {
	constructor({ generateId: generateId4 = generateId, id = generateId4(), transport = new DefaultChatTransport(), messageMetadataSchema, dataPartSchemas, state, onError, onToolCall, onFinish, onData, sendAutomaticallyWhen }) {
		...
		*/
		this.stop = async () => {
			...
			(_a24 = this.activeResumeRequest) == null || _a24.abortController.abort();
			...
			const runUpdateMessageJob = (job) => this.jobExecutor.run(() => {
				...
				return job({
					...
					write: ({ updateStatus = true } = {}) => {
						...
						if (response.state.message.id === ((_a25 = this.lastMessage) == null ? void 0 : _a25.id)) this.state.replaceMessage(this.state.messages.length - 1, response.state.message);
